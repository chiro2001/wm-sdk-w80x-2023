...
void spi_write_data(const u8 *data, u8 len) {
    tls_gpio_write(VFD_PIN_CS, 1);
    tls_gpio_write(VFD_PIN_CS, 0);
    _spi_write(data, len);
    tls_gpio_write(VFD_PIN_CS, 1);
...
void vfd_display_str(u8 addr, const char *str) {
    while (addr < VFD_DIGITS && str && *str) {
        spi_write_cmd(VFD_DCRAM_DATA_WRITE | addr, (const u8 *)str, 1);
        addr++;
        str++;
    }
}
...
void vfd_init(void) {
    ...
    // spi_set_mode(0);
    // spi_set_endian(0);
    // tls_spi_setup(TLS_SPI_MODE_0, TLS_SPI_CS_HIGH, VFD_SPI_CLK);
    ...
    // spi_write_data(cmds, sizeof(cmds) / sizeof(u8));
